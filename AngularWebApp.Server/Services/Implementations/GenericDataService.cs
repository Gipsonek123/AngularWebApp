...
namespace AngularWebApp.Server.Services.Implementations
{
    ...
    {
        private readonly AngularWebAppDbContext _dbContext;

        public GenericDataService(AngularWebAppDbContext dbContext)
        {
        ...
        public async Task<T> GetByUsername(string username)
        {
            ...
            if (entity == null)
            {
                throw new KeyNotFoundException($"Don't found object {typeof(T).Name} of Username {username}.");
            }
            return entity;
        }
